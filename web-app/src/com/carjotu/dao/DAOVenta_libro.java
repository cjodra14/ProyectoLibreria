...
public DAOVenta_libro(long isbn, int cantidad) {
	this.isbn=isbn;
	this.cantidad=cantidad;	
}
...
//	}
	...
	public static void insertarVenta(int npedido, String isbn, int cantidad) {
		try {
			String sqlQuery = "INSERT INTO libreria.venta_libro VALUES ('"+npedido+"','"+isbn+"', "+cantidad+");";
...
//MÉTODOS UTILITY DE CLASE
	...
	private static Vector<DAOVenta_libro> cargaResultSetToVector(ResultSet resultado) throws SQLException {
		...
		while(resultado.next()) {
			long isbn = resultado.getLong(2);
			int cantidad = resultado.getInt(1);
			ventaLibro = new DAOVenta_libro(isbn,cantidad);
	...
	public static Vector<DAOVenta_libro> listadoTOP() throws Exception {
		String consulta = "SELECT sum(cantidad), isbn from venta_libro group by isbn order by 1 desc";
		return buscaResultadosConConsulta(consulta);
	}
